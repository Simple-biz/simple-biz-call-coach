...
export function SimpleSuggestionDisplay({
  ...
  const getStageColor = (stage: string) => {
    switch (stage.toUpperCase()) {
      ...
      case 'CLOSING': return 'text-[#1B1F6B] bg-[#1B1F6B]/10 border-[#1B1F6B]/30'
  ...
  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex items-center gap-2 mb-1 px-1">
        <Sparkles className="w-3.5 h-3.5 text-[#1B1F6B]" />
        <span className="text-[10px] font-bold text-[#1B1F6B] uppercase tracking-widest">
          AI Suggested Line
        </span>
        {stage && (
        ...
        <div className="relative p-4 bg-white backdrop-blur-md border border-[#dddddd] rounded-xl shadow-lg">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 space-y-1.5 min-w-0">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-[#757575] uppercase tracking-tight">
                  {option?.label || 'Direct response'}
                </span>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-[#757575] font-mono">{model}</span>
                  <button
                    onClick={handleCopy}
            ...
            <div className="mt-3 pt-3 border-t border-[#dddddd]">
              <div className="flex items-center gap-3 text-[10px]">
                <div className="flex items-center gap-1">
                  <TrendingUp className="w-3 h-3 text-[#757575]" />
                  <span className="text-[#757575]">Performance:</span>
                </div>
                ...
                {performanceMetrics.cacheHitRate !== undefined && (
                  <div className="text-gray-400">
                    Cache: <span className="font-mono text-[#757575]">{Math.round(performanceMetrics.cacheHitRate * 100)}%</span>
                  </div>
                )}
                {performanceMetrics.aiLatency !== undefined && (
                  <div className="text-gray-500">
                    AI: <span className="font-mono text-[#757575]">{performanceMetrics.aiLatency}ms</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {!isLoading && (
            <div className="mt-4 flex gap-2">
              <button
                onClick={handleNext}
                className="flex-[3] flex items-center justify-center gap-2 py-2 px-3 bg-[#1B1F6B] hover:bg-[#14174f] text-white text-[11px] font-bold uppercase tracking-wider rounded-lg transition-all duration-200 active:scale-[0.97]"
              >
                Get Next Suggestion
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2 px-2 bg-[#F5F7FA] hover:bg-[#EFF3F6] text-[#333333] text-[11px] font-bold uppercase border border-[#dddddd] rounded-lg transition-all duration-200 active:scale-[0.95]"
