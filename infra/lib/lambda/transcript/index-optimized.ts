...
export const handler = async (
  ...
  try {
    ...
    console.log(`  - Cache Hit Rate: ${(aiTip.cacheHitRate * 100).toFixed(1)}%`);
    console.log(`  - Model Used: ${aiTip.model}`);
    ...
    if (aiLatency > CLAUDE_LATENCY_BUDGET_MS) {
      console.warn(`[Transcript] ⚠️ Claude latency exceeded budget: ${aiLatency}ms > ${CLAUDE_LATENCY_BUDGET_MS}ms`);
    }
    ...
    await saveAIRecommendation({
      conversation_id: connection.conversationId,
      heading: 'MARK_SCRIPT', // Indicates using Mark's scripts
